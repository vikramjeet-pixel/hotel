...
(function () {
    ...
    // ════════════════════════════════════════════════════════
    //  6. CORPORATE STAYS FORM (corporate-stays.html)
    // ════════════════════════════════════════════════════════
    if (document.getElementById('corp-enquiry-form')) {
        createSubmitHandler({
            formId: 'corp-enquiry-form',
            formType: 'corporate',
            successElId: 'corp-form-success',
            submitBtnSelector: '#corp-submit-btn',
            ...
            validateFields: function () {
                ...
                const validators = {
                    'corp-firstname': { test: v => v.trim().length >= 2, msg: 'Please enter your first name.' },
                    'corp-lastname': { test: v => v.trim().length >= 2, msg: 'Please enter your last name.' },
                    'corp-email': { test: v => emailRegex.test(v.trim()), msg: 'Please enter a valid email address.' },
                    'corp-phone': { test: v => v.trim().length >= 7, msg: 'Please enter a valid phone number.' },
                    'corp-group-type': { test: v => v !== '', msg: 'Please select group type.' },
                    'corp-rooms-needed': { test: v => v !== '', msg: 'Please select rooms required.' },
                    'corp-arrival': { test: v => { if (!v) return false; return new Date(v) >= new Date(new Date().toDateString()); }, msg: 'Please select a future arrival date.' },
                    'corp-nights': { test: v => v !== '', msg: 'Please select nights.' },
                };
            ...
            collectData: function () {
                const firstName = document.getElementById('corp-firstname')?.value?.trim() || '';
                const lastName = document.getElementById('corp-lastname')?.value?.trim() || '';
                const groupTypeSelect = document.getElementById('corp-group-type');
                const roomsSelect = document.getElementById('corp-rooms-needed');
                const nightsSelect = document.getElementById('corp-nights');
                const notes = document.getElementById('corp-notes')?.value?.trim() || 'No special requirements specified.';

                const groupType = groupTypeSelect?.options[groupTypeSelect.selectedIndex]?.text || '';
                const roomsNeeded = roomsSelect?.options[roomsSelect.selectedIndex]?.text || '';
                const arrival = document.getElementById('corp-arrival')?.value || '';
                const nights = nightsSelect?.options[nightsSelect.selectedIndex]?.text || '';
                const company = document.getElementById('corp-company')?.value?.trim() || 'None specified';

                const msgText = `Corporate stay enquiry for ${company}. Group Type: ${groupType}, Rooms Needed: ${roomsNeeded}, Arrival: ${arrival}, Nights: ${nights}. Special requests: ${notes}`;
            ...
            resetFields: function () {
                const form = document.getElementById('corp-enquiry-form');
                form?.querySelectorAll('.ev-form__input, .ev-form__select, .ev-form__textarea').forEach(el => {
                    el.value = '';
                    el.classList.remove('is-error');
                });
